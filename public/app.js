const socket = io();
let state = null, current = 'headline', selectedOrder = [];
const $ = (s) => document.querySelector(s);
const rooms = [
  ['headline', '01', 'El titular'], ['photo', '02', 'La fotografía'], ['social', '03', 'La red social'],
  ['puzzle', '04', 'El rompecabezas'], ['detective', '05', 'El detective'], ['classify', '06', 'Verdad o engaño'], ['final', '∞', 'La edición']
];
const messages = [
...
function name() { return $('#player-name').value.trim() || 'Periodista'; }
...
function isUnlocked(id, i) {
  if (id === 'classify') return state.solved.length >= 5;
  if (id === 'final') return state.solved.length >= 5 && state.classifications.done;
  return i === 0 || solved(rooms[i - 1][0]);
}
function drawGame() {
  if (!state?.started) return;
  $('#progress').textContent = `${state.solved.length + (state.classifications.done ? 1 : 0)} / 6`;
...
function renderScene() {
  ...
  if (current === 'headline') scene.innerHTML = header('01', 'El titular', 'Activa el detector de exageraciones: pulsa la palabra que cambia una posibilidad por una certeza.') + `<article class="paper"><small class="eyebrow">ALERTA CLIMÁTICA · 23:21</small><div class="headline">CIENTÍFICOS <span class="scan-word" data-solve="headline">CONFIRMAN</span> QUE EL PLANETA PODRÍA SER INHABITABLE EN 10 AÑOS</div><p>Un informe modela un escenario hipotético de riesgo alto. No presenta una predicción ni una fecha confirmada.</p><div class="source-line">FUENTE ORIGINAL: Informe de escenarios climáticos, sección 4.2</div><div class="clue">Encuentra la palabra que el editor añadió para hacer el titular más alarmante.</div></article>`;
  if (current === 'photo') scene.innerHTML = header('02', 'La fotografía', 'Haz de detective visual: selecciona la imagen que coincide con la fecha, el lugar y el hecho de la noticia.') + `<div class="photo-grid"><div class="photo choice-photo" data-photo="wrong"><b>Imagen A</b><span class="hotspot" style="top:42%;left:47%">2017</span><span class="hotspot" style="top:18%;right:12%">PUERTO NORTE</span></div><div class="photo two choice-photo" data-photo="right"><b>Imagen B</b><span class="hotspot" style="top:40%;left:45%">2024</span><span class="hotspot" style="bottom:29%;right:14%">BARRIO CENTRAL</span></div></div><div class="clue">Noticia: evacuación preventiva en Barrio Central, 24 de julio de 2024. Elige la imagen correcta.</div>`;
  if (current === 'social') scene.innerHTML = header('03', 'La red social', 'Elige la publicación que merece entrar al expediente. No te guíes por la cantidad de compartidos.') + `<div class="post play-post" data-post="bad"><b>@alerta_ahora</b><p>🔥 ¡INCREÍBLE! Nadie está hablando de esto.</p><small>12.4 mil compartidos · enlace sin autor</small></div><div class="post play-post" data-post="bad"><b>@datos_sin_filtro</b><p>😱 Esto demuestra que los medios nos están mintiendo.</p><small>8.1 mil compartidos · captura recortada</small></div><div class="post verified play-post" data-post="good"><b>@observatorio_publico ✓</b><p>Los datos publicados muestran una variación mensual. Consulta la metodología completa.</p><small>Enlace al informe original · institución identificable · 08:14</small></div>`;
...
function bindScene() {
  document.querySelectorAll('[data-solve="headline"]').forEach(el => el.onclick = () => submitPuzzle('headline', 'CONFIRMAN'));
  document.querySelectorAll('[data-photo]').forEach(el => el.onclick = () => { if (el.dataset.photo === 'right') { el.classList.add('selected'); submitPuzzle('photo', '2417'); } else toast('La fecha y el lugar no coinciden. Sigue buscando.'); });
  document.querySelectorAll('[data-post]').forEach(el => el.onclick = () => { if (el.dataset.post === 'good') { el.classList.add('selected'); submitPuzzle('social', 'FUENTE'); } else toast('Esa publicación no permite rastrear una fuente original.'); });
