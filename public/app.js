...
function bindScene() {
  ...
  document.querySelectorAll('[data-post]').forEach(el => el.onclick = () => { const type = el.dataset.post; socialChecks.add(type); el.classList.add('selected'); const notes = {viral:'El enlace termina en una página sin autor ni documento.',capture:'La captura no permite comprobar fecha, autor ni origen.',source:'El enlace lleva a la metodología y al informe original.'}; $('#social-clue').textContent = `${notes[type]} Fichas auditadas: ${socialChecks.size} / 3.`; if (socialChecks.size === 3 && type === 'source') submitPuzzle('social', 'FUENTE'); else if (socialChecks.size === 3) toast('Ahora seleccionen la ficha que sí enlaza a la fuente original.'); });
